...
LOOP:	
		...
		LDR	R3, COUNT
		CMP	R3, #3
		BEQ	LIGHT			//0.25 first time to light
		CMP	R3, #2
		BEQ	LIGHT			//0.25 second time to light
		MOV	R3, #0
		STR	R3, [R5]		//the rest 0.25*2 to dark
		B LOOP
		
LIGHT:	MOV	R3, #1
		STR	R3, [R5]		//total 0.25*2 to light
		B LOOP
...
CONFIG_TIMER:	
		...
		//start the privite timer, enable interrupts
		MOV	R1, #0b111
		STR	R1, [R0, #0x8]
