...
/* Configure the KEY pushbuttons port to generate interrupts */
              	LDR		R1, =0xFF200050		//SET MASK FOR KEY
				MOV		R0, #0xF
				STR		R0, [R1, #0x8]
				
/* Enable IRQ interrupts in the ARM processor */
                MOV		R0, #0b01010011		//SET MASK FOR IRQ
				MSR		CPSR, R0
...
KEY_ISR:		PUSH	{R0-R7, LR}				//DETERMINE WHICH KEY IS PRESSED
				LDR		R0, =0xFF200050
				LDR		R1, [R0,#0xC]
				CMP		R1,	#0b1000
				LDREQ	R2, =0x4F000000
				...
				LDR		R3, =0xFF200020			//SET FOR DISPLAY
				LDR		R4, [R3]
				EOR		R4, R2
				STR		R4, [R3]
				
				LDR		R1, =0xFFFFFFFF			//RESET THE EDGE CAPTURE
				STR  	R1, [R0, #0xC]
				POP		{R0-R7, PC}
