...
CONFIG_TIMER:                             
				...
				STR		R1, [R0, #0x8]
				LDR		R1, =0x017D
				STR		R1, [R0, #0xC]			//COUNT DOWN FROM 25X10^6
				LDR		R1, =0b0111				//SET CONTROL
				STR		R1, [R0, #0x4]
				LDR		R1, [R0]
				POP		{R0-R2, LR}
                BX      LR                  
...
CONFIG_KEYS:       
				...
				LDR		R1, =0xFF200050
				MOV		R0, #0xF
				STR		R0, [R1,#0x8]			//SET EDGE CAPTURE FOR KEYS
				POP		{R0,R1, LR}
                BX      LR       				   
...
KEYS_HANDLER:                       
                CMP      R5, #73         // check the interrupt ID TO BE KEY
				BLEQ	 KEY_ISR
				CMP		 R5, #72		 // check the interrupt ID TO BE TIMER
				BLEQ	 TIMER_ISR
...
//KEY_ISR & TIMER_ISR
TIMER_ISR:		
				LDR 	R0, =0xFF202000				//RESET THE TIMER TO
				MOV		R1, #0
				STR		R1, [R0]
				
				LDR		R0, =RUN					// RUN DETERMINED BY KEY PRESSED OR UNPRESSED
				LDR		R0, [R0]
				LDR		R1, =COUNT					// COUNT FROM THE TIMER
				LDR		R2, [R1]
				ADD		R0, R2						// GO COUNT IF(RUN & COUNT)
				STR		R0, [R1]
				BX 		LR
				
KEY_ISR:		
				LDR 	R0, =0xFF200050				//RESET THE EDGE CAPTURE
				LDR		R1, [R0, #0xC]
				LDR		R2, =0xFFFFFFFF				//RESET THE EDGE CAPTURE
				STR		R2, [R0, #0xC]
...
KEY_0_PRESSED:	PUSH	{R0-R2, LR}	

				LDR		R1, =RUN					//SET RUN SIGNAL
				LDR		R0, [R1]
				MOV		R2, #1
				EOR		R0, R2
				STR		R0, [R1]
...
KEY_1_PRESSED:	PUSH	{R0-R5, LR}	
				LDR		R0, =0xFF202000
				MOV		R1, #0b1000				//STOP COUNTER
				STR		R1, [R0, #0x4]

				LDR		R2, [R0, #0x8]			//GET THE CURRENT SPEED
				LDR		R3, [R0, #0xC]
				LSL		R3, #16
				ORR		R2, R3
				LSR		R2, #1
				
				LDR		R3, =0xFFFF0000			//UPDATE THE SPEED
				ADD		R4, R2, R3
				LDR		R3, =0x0000FFFF
				ADD		R5, R2, R3
				LSR		R4, #16
