...
_start:	
		...
		//enable timer interupts in th GIC
		BL	CONFIG_GIC
		BL	CONFIG_TIMER
		...
		//for reading sw address and display in hex
		LDR R6, =0xFF200040 	//SW address in R6
		LDR	R7, =0xFF200020
		...
		MOV	R9, #0
LOOP:	
			LDR	R0, [R6]			//Read SW value to R0
			
			// to get value stored in R4 & R5
			CMP	R0, #0
			BEQ	DISPLAY_0
			...
			//to display
DISPLAY:	LDR	R3, COUNT
			CMP	R3, #0
			BEQ	DARK			//0.25 first time to dark
			CMP	R3, #1
			BEQ	DARK			//0.25 second time to dark
			
			STR	R4, [R7]		//the rest 0.25*2 to light
			STR	R5, [R7, #0x10]	
			B LOOP

DARK:		STR	R8, [R7]		//total 0.25*2 to dark
			STR	R9, [R7, #0x10]	
			MOV	R8, R4
			MOV	R9, R5
			B LOOP
...
CONFIG_TIMER:	
		...
		//start the interval timer, enable interrupts
		MOV	R1, #0b0111
		STR	R1, [R0, #0x8]
...
SERVICE_IRQ:
		//IRQ Exception service routine, to read check what interrupt it is
		PUSH 	{R0-R7, LR}
		LDR		R4, =0xFFFEC100
		LDR		R5, [R4, #0x0C]		//read the interrupt ID

		CMP		R5, #29				//check if the interrupt is the timer interrupt
UNEXPECTED:		BNE		UNEXPECTED	//STOP for unknown execptions
...
TIMER_ISR:	
		//what should i do if it is a timer interrupt
		LDR		R0, =0xFFFEC600
		
		MOV		R1, #1
		STR		R1, [R0, #0xC]	//store 1 to clear the interrupt
	
		LDR		R2, =COUNT 	//change the count to let it be 0-1-2-3
		LDR		R3, [R2]
		ADD		R3, #1
		CMP		R3, #4
		SUBEQ	R3, #4
